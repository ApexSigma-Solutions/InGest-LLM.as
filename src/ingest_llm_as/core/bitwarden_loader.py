...
class SecretMapping:
    ...
    secret_id: str  # Bitwarden secret ID (e.g., from INGEST_LINEAR_WEBHOOK_SECRET_PRD_ID)
...
class BitwardenSecretLoader:
    ...
    def __init__(self, access_token: Optional[str] = None):
        ...
        """
        self.access_token = access_token or os.getenv("INGEST_BWS_ACCESS_TOKEN") or os.getenv("BWS_ACCESS_TOKEN")
...
def create_secret_mappings_from_env() -> List[SecretMapping]:
    ...
    for secret_id_var, target_var, required in mapping_definitions:
        ...
        if secret_id:
            mappings.append(SecretMapping(secret_id=secret_id, env_var=target_var, required=required))
...
def load_bitwarden_secrets() -> None:
    ...
    if not access_token:
        logger.info("No Bitwarden access token found - skipping Bitwarden secret loading")
        return
    ...
    if not loader.enabled:
        logger.warning("Bitwarden loader not enabled - using environment variables only")
        return
    ...
    if not mappings:
        logger.warning("No secret mappings found - create INGEST_*_PRD_ID variables for Bitwarden secrets")
        return
