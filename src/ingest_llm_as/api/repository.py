...
from ..services.memos_client import get_memos_client, MemOSClient, MemOSConnectionError, MemOSAPIError
...
async def ingest_python_repository(
    ...
    if langfuse_client.enabled:
        trace_id = langfuse_client.create_trace(
            ...
            tags=["repository", "ingestion", "api", request.repository_source.value],
            input_data={
                "repository_source": request.repository_source.value,
                "source_path": request.source_path,
                "max_files": request.max_files,
    ...
    try:
        ...
        files_processed = len(response.files_processed) if response.files_processed else 0
        ...
        if langfuse_client.enabled and trace_id:
            success_rate = 1.0 if response.status == ProcessingStatus.COMPLETED else 0.5 if response.status == ProcessingStatus.PENDING else 0.0
            ...
            if response.files_to_process > 0:
                size_score = min(1.0, response.files_to_process / 100)  # Normalize to reasonable repo size
                langfuse_client.score_trace(
                    trace_id=trace_id,
        ...
        logger.error(f"memOS.as connection error in repository ingestion {ingestion_id}: {e}")
        raise HTTPException(
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error during repository ingestion"
        )


@router.get("/python-repo/status/{ingestion_id}", response_model=RepositoryIngestionResponse)
async def get_repository_ingestion_status(ingestion_id: str) -> RepositoryIngestionResponse:
    """
...
async def analyze_repository(
    ...
    try:
        ...
        analysis = _perform_repository_analysis(ingestion_response, request.analysis_type)
...
def _perform_repository_analysis(
    ingestion_response: RepositoryIngestionResponse,
    analysis_type: str
) -> Dict[str, Any]:
    """
    ...
    total_loc = sum(result.file_size // 50 for result in files_processed if result.status == ProcessingStatus.COMPLETED)  # Rough estimate
    
    # Documentation coverage (files with docstrings vs total)
    python_files = [r for r in files_processed if r.relative_path.endswith('.py') and r.status == ProcessingStatus.COMPLETED]
    documented_files = [r for r in python_files if r.elements_extracted > 0]  # Assume extracted elements indicate documentation
    ...
    test_coverage_estimate = min(1.0, len(test_files) / len(python_files)) if python_files else 0.0
    ...
    if summary.average_complexity > 5.0:
        ...
        refactoring_opportunities.append("Reduce cyclomatic complexity of complex functions")
    ...
    if summary.total_files_failed > 0:
        suggestions.append(f"Fix {summary.total_files_failed} files that failed processing")
    
    # Large file analysis
    large_files = [f for f in summary.largest_files if f.get("size", 0) > 50000]  # > 50KB
    if large_files:
        suggestions.append(f"Consider splitting {len(large_files)} large files")
        refactoring_opportunities.extend([f"Split large file: {f['path']}" for f in large_files[:3]])
