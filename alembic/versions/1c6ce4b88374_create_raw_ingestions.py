...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('ingestion_id', sa.dialects.postgresql.UUID(as_uuid=True), nullable=False, unique=True),
    ...
    op.create_index('idx_raw_ingestions_ingestion_id', 'raw_ingestions', ['ingestion_id'], unique=True)


def downgrade() -> None:
    """Drop raw_ingestions table and indexes."""
    # Explicitly drop indexes before dropping the table
