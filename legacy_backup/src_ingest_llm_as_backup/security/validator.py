...
SECRET_PATTERNS = {
    "api_key": re.compile(r"^(api[_-]?key|apikey)[\s=:]+[\"']?[a-zA-Z0-9]{20,}[\"']?", re.IGNORECASE),
    "password": re.compile(r"^(password|passwd|pwd)[\s=:]+[\"']?[^\s\"']{8,}[\"']?", re.IGNORECASE),
    "secret": re.compile(r"^(secret|token|jwt)[\s=:]+[\"']?[^\s\"']{10,}[\"']?", re.IGNORECASE),
...
def is_secret_variable(var_name: str) -> bool:
    ...
    secret_keywords = [
        ...
        "api_key", "jwt", "private", "credential", "auth",
    ]
