...
class TestForwarderSettings:
    ...
    def test_custom_settings_from_env(self, mock_validate):
        ...
        with patch.dict(
            "os.environ",
            {"FORWARDER_INGEST_LLM_URL": "http://custom-ingest:9000", "FORWARDER_FORWARDER_TIMEOUT": "10"},
            clear=True,
        ):
            settings = ForwarderSettings()
...
class TestForwardLinearWebhookHeaders:
    ...
    async def test_forward_webhook_preserves_headers(
        ...
        assert captured_post_call["headers"]["X-Correlation-ID"] == sample_correlation_id
