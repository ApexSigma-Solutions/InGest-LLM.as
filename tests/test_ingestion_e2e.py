...
MEMOS_API_BASE_URL = "http://localhost:8091"
REQUEST_TIMEOUT = 30
...
class TestTextIngestion:
    ...
    async def test_basic_text_ingestion(self, sample_text_content, ingestion_metadata):
        ...
        async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT) as client:
            ...
            assert len(ingestion_response["results"]) == ingestion_response["total_chunks"]
    ...
    async def test_async_text_ingestion(self, sample_text_content, ingestion_metadata):
        ...
        async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT) as client:
            ...
            # ingestion_id = ingestion_response["ingestion_id"]
            # await self._wait_for_completion(client, ingestion_id)
        ...
        async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT) as client:
            ...
            for memory_id in memory_ids:
                memory_response = await client.get(
                    f"{MEMOS_API_BASE_URL}/memories/{memory_id}"
                )
                ...
                if memory_response.status_code == 200:
                    ...
                    pytest.fail(f"Unexpected response from memOS.as: {memory_response.status_code}")
    
    @pytest.mark.asyncio
    async def test_memory_search_integration(self, sample_text_content, ingestion_metadata):
        ...
        async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT) as client:
            ...
            search_response = await client.get(
                f"{MEMOS_API_BASE_URL}/memories/search",
            ...
            if search_response.status_code == 200:
                ...
                print(f"Search failed with status {search_response.status_code}: {search_response.text}")
