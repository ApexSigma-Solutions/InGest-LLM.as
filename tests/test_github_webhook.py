...
class TestGitHubWebhookIntegration:
    ...
    @patch.dict(os.environ, {"GITHUB_WEBHOOK_SECRET": "test-secret", "POSTGRES_DSN": "postgresql://user:pass@localhost:5432/db"})
    def test_webhook_valid_signature(self, client):
        ...
        with patch("ingest_llm_as.routers.webhook.saga.begin_transaction", new_callable=AsyncMock) as mock_begin, \
             patch("ingest_llm_as.routers.webhook.saga.commit_transaction", new_callable=AsyncMock) as mock_commit, \
             patch("ingest_llm_as.routers.webhook._process_github_webhook_payload", new_callable=AsyncMock) as mock_process:
    ...
    def test_webhook_circuit_breaker_open(self, client):
        ...
        with patch("ingest_llm_as.routers.webhook.github_circuit_breaker.is_closed", return_value=False):
            response = client.post(
                "/api/v1/webhook/github",
                json=GITHUB_PAYLOAD,
    ...
    def test_webhook_processing_failure_dlq(self, client):
        ...
        with patch("ingest_llm_as.routers.webhook.saga.begin_transaction", new_callable=AsyncMock), \
             patch("ingest_llm_as.routers.webhook._process_github_webhook_payload", side_effect=Exception("Processing failed")) as mock_process, \
             patch("ingest_llm_as.routers.webhook.write_to_dlq", new_callable=AsyncMock) as mock_dlq, \
             patch("ingest_llm_as.routers.webhook.record_dlq_message") as mock_record_dlq, \
             patch("ingest_llm_as.routers.webhook.record_webhook_request") as mock_record_request:
